...
def load_model_decode(data, name):
    ...
    pred_results = evaluate(data, model, name, data.nbest)
    end_time = time.time()
...
def main_recog(tokens):
    root = '/home/ec2-user/'
    os.chdir(os.path.join(root, 'APIRecognition'))
# if __name__ == '__main__':
    ...
    if status == 'train':
        ...
        # exit(0)
        #data.show_data_summary()
        data.generate_instance_from_sentence(tokens, 'raw')
        ...
        preds, contents = data.get_nbest_decoded_results(decode_results, 'raw')

        return contents, preds
            # print(contents)
            # print(preds)
    else:
        print("Invalid argument! Please use valid arguments! (train/test/decode)")

# tokens = "However , torch.tensors are designed to be used in the context of gradient descent optimization".split()
# main(tokens)