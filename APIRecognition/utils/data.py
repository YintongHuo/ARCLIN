...
class Data:
    ...
    def generate_instance_from_sentence(self, tokens, name):
        self.fix_alphabet()
        if name == "train":
            self.train_texts, self.train_Ids = read_instance_from_sentence(self.train_dir, self.word_alphabet, self.char_alphabet, self.feature_alphabets, self.label_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH, self.sentence_classification, self.split_token)
        elif name == "dev":
            self.dev_texts, self.dev_Ids = read_instance_from_sentence(self.dev_dir, self.word_alphabet, self.char_alphabet, self.feature_alphabets, self.label_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH, self.sentence_classification, self.split_token)
        elif name == "test":
            self.test_texts, self.test_Ids = read_instance_from_sentence(self.test_dir, self.word_alphabet, self.char_alphabet, self.feature_alphabets, self.label_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH, self.sentence_classification, self.split_token)
        elif name == "raw":
            self.raw_texts, self.raw_Ids = read_instance_from_sentence(tokens, self.word_alphabet, self.char_alphabet, self.feature_alphabets, self.label_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH, self.sentence_classification, self.split_token)
        else:
            print("Error: you can only generate train/dev/test instance! Illegal input:%s"%(name))
    ...
    def get_nbest_decoded_results(self, predict_results, name):
        ## predict_results : [whole_sent_num, nbest, each_sent_length]
        ## pred_scores: [whole_sent_num, nbest]
        ...
        for idx in range(sent_num):
            ...
            for idy in range(sent_length):
                try:  # Will fail with python3
                    label_string = content_list[idx][0][idy].decode('utf-8')
                except:
                    label_string = content_list[idx][0][idy]
                count = 0
                for idz in range(nbest):
                    if predict_results[idx][idz][idy] == 'B':
                        count += 1
                ...
                if count >= 2:
                    ...
                    tmp_preds.append('0')
                tmp_contents.append(label_string)
            binary_preds.append(tmp_preds)
            binary_contents.append(tmp_contents)
        return binary_preds, binary_contents
